...
st.set_page_config(page_title="VRC / HRV RRi Analyzer Pro v6.2 Kubios Mode", layout="wide")
...
def domains_figure(metrics_df, method="median", title="Dominios Amplitud / Vagal / Complejidad / Recurrencia"):
    ...
    phases = list(dom.index)

    for col in dom.columns:
        y = dom[col].astype(float).values
        fig.add_trace(go.Bar(x=phases, y=y, name=f"{col} barras", opacity=0.38))
        fig.add_trace(go.Scatter(x=phases, y=y, mode="lines+markers+text", name=col, text=[f"{v:.1f}" if pd.notna(v) else "" for v in y], textposition="top center", line=dict(width=3)))
...
def mse_figure(metrics_df, title="MSE 1-20"):
    ...
    # Barras finas + líneas por escala MSE
    for scale, col in enumerate(mse_cols, start=1):
        y = [metrics_df.loc[ph, col] if ph in metrics_df.index else np.nan for ph in phases]
        fig.add_trace(go.Bar(x=phases, y=y, name=col, opacity=0.18, showlegend=False))
        fig.add_trace(go.Scatter(x=phases, y=y, mode="lines+markers", name=col, line=dict(width=1.8), marker=dict(size=5)))
...
def mse_compare_figure(long_df, phases, scales=None):
    ...
    for rec in records_order:
        ...
        for ph in phases:
            ...
            fig.add_trace(go.Scatter(x=x, y=y, mode="lines+markers", name=f"{rec} · {ph}"))
...
def calculate_all(rr, include_rqa=True, include_hvg=False):
    ...
    out.update(mse_metrics(rr, max_scale=20))
...
def comparison_bar_line(pivot, variable):
    ...
    if len(phases) == 1:
        ph = phases[0]
        x_labels = [_short_record_label(rec, 24) for rec in cols_sorted]
        y_vals = [pivot.loc[ph, rec] for rec in cols_sorted]

        fig.add_trace(go.Bar(x=x_labels, y=y_vals, name=f"{variable} · barras", opacity=0.68))
        fig.add_trace(go.Scatter(x=x_labels, y=y_vals, mode="lines+markers", name=f"{variable} · tendencia", line=dict(width=3)))
        fig.update_layout(
            height=540,
            title=f"{variable}: evolución cronológica en {ph}",
            xaxis_title="Registro ordenado cronológicamente",
            yaxis_title=variable,
            hovermode="x unified",
        )
        return fig

    for rec in cols_sorted:
        y = pivot[rec].astype(float)
        fig.add_trace(go.Bar(x=phases, y=y, name=f"{_short_record_label(rec)} · barras", opacity=0.60))
        fig.add_trace(go.Scatter(x=phases, y=y, mode="lines+markers", name=f"{_short_record_label(rec)} · tendencia", line=dict(width=3)))

    fig.update_layout(
        height=540,
        title=f"{variable}: fases + tendencia por registro",
        xaxis_title="Fase",
        yaxis_title=variable,
        barmode="group",
        hovermode="x unified",
...
def dashboard_bar_smooth(long_df, phases, params):
    """
    Dashboard evolutivo:
    - registros ordenados cronológicamente de más antiguo a más reciente,
    - si hay una sola fase: eje X = registro/fecha, línea de tendencia real,
    - si hay varias fases: eje X = fase + registro, pero siempre en orden cronológico,
    - altura aumenta con el número de parámetros para evitar paneles pequeños.
    """
    params = [p for p in params if p in long_df.columns]
    phases = [p for p in phases if p in PHASES]

    if len(params) == 0 or len(phases) == 0 or long_df.empty:
        fig = go.Figure()
    ...
    for idx, param in enumerate(params):
        ...
        if one_phase:
            ...
            for rec in records_order:
                if rec in d.index:
                    x_labels.append(_short_record_label(rec, 22))
                    y_vals.append(d.loc[rec, param] if param in d.columns else np.nan)
            ...
            fig.add_trace(
                go.Bar(
                    ...
                    opacity=0.66,
                    showlegend=False,
                    customdata=[[ph, rec] for rec in x_labels],
                    hovertemplate=f"Registro: %{{x}}<br>Fase: {ph}<br>{param}: "+"%{y:.3f}<extra></extra>",
                ),
                row=r,
                col=c
            )
            ...
            fig.update_xaxes(title_text="Registro ordenado cronológicamente", tickangle=0, row=r, col=c)

        else:
            # Múltiples fases: cada fase contiene registros en orden cronológico.
            x_labels, y_vals, custom = [], [], []
            for ph in phases:
                d = dfp[dfp["Fase"] == ph].set_index("Registro")
                for rec in records_order:
                    if rec in d.index:
                        x_labels.append(f"{ph}<br>{_short_record_label(rec, 18)}")
                        y_vals.append(d.loc[rec, param] if param in d.columns else np.nan)
            ...
            # Líneas por fase para ver evolución cronológica dentro de cada fase.
            for ph in phases:
                d = dfp[dfp["Fase"] == ph].set_index("Registro")
                x_line, y_line = [], []
                for rec in records_order:
                    if rec in d.index:
                        x_line.append(f"{ph}<br>{_short_record_label(rec, 18)}")
                        y_line.append(d.loc[rec, param] if param in d.columns else np.nan)
                if len(x_line) >= 1:
                    fig.add_trace(
                        go.Scatter(
                            x=x_line,
                            ...
                            line=dict(width=3),
                            marker=dict(size=6),
                            hovertemplate=f"{param}: "+"%{y:.3f}<extra></extra>",
                        ),
                        row=r,
                        col=c
                    )

            fig.update_xaxes(title_text="Fase · registro cronológico", tickangle=0, row=r, col=c)

        fig.update_yaxes(title_text=param, row=r, col=c)

    fig.update_layout(
        height=max(650, rows * 520),
        title="Dashboard evolutivo: barras + línea de tendencia cronológica",
        hovermode="closest",
        bargap=0.25,
    )
    return fig
...
st.title("VRC / HRV RRi Analyzer Pro v6.2 · Kubios Mode")
st.caption("Segmentación tipo Kubios, dominios autonómicos, MSE 1-20, dashboard evolutivo e informe automático.")
